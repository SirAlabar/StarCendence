...
export default class PongPage extends BaseComponent 
{
 ...
 render(): string 
 {
  ...
  mount(): void 
  {
        ...
        const exitbutton = document.getElementById("exitGameBtn");

        if (start2D) start2D.addEventListener("click", () => this.start2DPong());
        if (start3D) start3D.addEventListener("click", () => this.start3DPong());
        if (backBtn) backBtn.addEventListener("click", () => this.goBack());
        if (exitbutton) exitbutton.addEventListener("click", ()=> this.goBack());
  }
  ...
  private start2DPong(): void 
  {
    ...
    if (canvas) 
    {
            //resizing canvas for game and removing menu
            this.resize(canvas);
  ...
  private goBack(): void            //back buttom ft
  {
        this.dispose();
        navigateTo('/games');
  }

  public dispose(): void            //remove and clean the pongscene
  {
        this.pongScene = null;
  }
}
