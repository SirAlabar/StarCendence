...
private updatePhysics(): void 
{
  ...
  /**
   * Get pod position (for multiplayer broadcasting)
   */
  public getPosition(podId: string): Vector3 | null 
  {
    const podData = this.pods.get(podId);
    if (!podData) 
    {
    ...
    return podData.mesh.position.clone();
  }

  /**
   * Get pod rotation (for multiplayer broadcasting)
   */
  public getRotation(podId: string): Quaternion | null 
  {
    const podData = this.pods.get(podId);
    if (!podData || !podData.mesh.rotationQuaternion) 
    {
      return null;
    }
    return podData.mesh.rotationQuaternion.clone();
  }

  /**
   * Get pod velocity (for multiplayer broadcasting)
   */
  public getVelocity(podId: string): Vector3 | null 
  {
    const podData = this.pods.get(podId);
    if (!podData) 
    {
  ...
  /**
   * Add collision mesh to physics world
   */
  public addCollisionMesh(collisionMesh: AbstractMesh): void 
  {
    if (!this.isInitialized || !this.ammoInstance) 
    {
      throw new Error('RacerPhysics: Cannot add collision mesh - not initialized');
    }
    ...
    const shape = new Ammo.btBvhTriangleMeshShape(mTriMesh, true, true);
