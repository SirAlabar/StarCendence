...
export class Hero extends BaseComponent {
  ...
  mount(): void {
    ...
    // Tamanho interno do buffer (real pixels)
    ...
    const fragment = `
      ...
      void main(){
        ...
        vec4 n2 = texture2D(texNeb2, uv - vec2(cos(time*0.018), sin(time*0.022))*0.02);

        float mask = blob(vUV, mouse, time) * reveal;
        vec3 mixNeb = mix(n1.rgb, n2.rgb, 0.5 + 0.5*sin(time*0.3));
    ...
    const texNeb2 = this.loadTexture('/assets/images/backgrounds/nebula2.jpg');
    ...
    const loop = () => {
      ...
      gl.activeTexture(gl.TEXTURE2); gl.bindTexture(gl.TEXTURE_2D, texNeb2);
